...
def main():
    df_exchange = pd.read_csv(
        'data/exchange.csv', encoding='cp932', header=1, names=['date', 'USD', 'rate'], index_col=0, parse_dates=True
    )

    df_jgbcm = pd.read_csv(
        'data/jgbcm_all.csv', encoding='cp932', header=1, index_col=0, parse_dates=True, date_parser=parse_japanese_date, na_values=['-']
    )

    df_jobs = pd.read_excel(
        'data/第3表.xlsx', skiprows=3, skipfooter=3, usecols='C,E:P', index_col=0
    )

    df_jobs.columns = [c.split('.')[0] for c in df_jobs.columns]
    s_jobs = df_jobs.stack()
    print(s_jobs, s_jobs.index)
    ...
    plt.subplot(3, 1, 3)
    plt.plot(s_jobs.index, s_jobs, label='有効求人率(季節調整値)')
    plt.xlim(min_date, max_date)
    plt.ylim(0.0, 2.0)
